...
class GraphCache
{
    friend RMW_DDS_COMMON_PUBLIC std::ostream& operator<<(std::ostream& ostream, const GraphCache& topic_cache);
...
struct EntityInfo
{
    ...
    EntityInfo(const std::string& topic_name,
               ...
               const rmw_gid_t& participant_gid,
               const rmw_qos_profile_t& qos)
        : topic_name(topic_name), topic_type(topic_type), participant_gid(participant_gid), qos(qos)
    {
    }
};
