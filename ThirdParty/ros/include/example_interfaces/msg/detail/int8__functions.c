  ...
  example_interfaces__msg__Int8__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    example_interfaces__msg__Int8 * data =
      (example_interfaces__msg__Int8 *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!example_interfaces__msg__Int8__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          example_interfaces__msg__Int8__fini(&data[i]);
        }
        free(data);
        return false;
      }
