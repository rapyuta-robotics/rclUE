  ...
  example_interfaces__msg__UInt32__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    example_interfaces__msg__UInt32 * data =
      (example_interfaces__msg__UInt32 *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!example_interfaces__msg__UInt32__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          example_interfaces__msg__UInt32__fini(&data[i]);
        }
        free(data);
        return false;
      }
