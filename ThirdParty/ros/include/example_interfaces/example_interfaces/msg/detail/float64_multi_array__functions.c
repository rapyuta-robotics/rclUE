  ...
  example_interfaces__msg__Float64MultiArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    example_interfaces__msg__Float64MultiArray * data =
      (example_interfaces__msg__Float64MultiArray *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!example_interfaces__msg__Float64MultiArray__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          example_interfaces__msg__Float64MultiArray__fini(&data[i]);
        }
        free(data);
        return false;
      }
