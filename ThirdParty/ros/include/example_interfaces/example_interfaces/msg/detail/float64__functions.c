  ...
  example_interfaces__msg__Float64__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    example_interfaces__msg__Float64 * data =
      (example_interfaces__msg__Float64 *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!example_interfaces__msg__Float64__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          example_interfaces__msg__Float64__fini(&data[i]);
        }
        free(data);
        return false;
      }
