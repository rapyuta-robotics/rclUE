  ...
  simulation_interfaces__msg__EntityInfo__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    simulation_interfaces__msg__EntityInfo * data =
      (simulation_interfaces__msg__EntityInfo *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!simulation_interfaces__msg__EntityInfo__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          simulation_interfaces__msg__EntityInfo__fini(&data[i]);
        }
        free(data);
        return false;
      }
