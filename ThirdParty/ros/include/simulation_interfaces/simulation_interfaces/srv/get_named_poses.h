// generated from rosidl_generator_c/resource/idl.h.em
// with input from simulation_interfaces:srv/GetNamedPoses.idl
// generated code does not contain a copyright notice

#ifndef SIMULATION_INTERFACES__SRV__GET_NAMED_POSES_H_
#define SIMULATION_INTERFACES__SRV__GET_NAMED_POSES_H_

#include "simulation_interfaces/srv/detail/get_named_poses__struct.h"
#include "simulation_interfaces/srv/detail/get_named_poses__functions.h"
#include "simulation_interfaces/srv/detail/get_named_poses__type_support.h"

#endif  // SIMULATION_INTERFACES__SRV__GET_NAMED_POSES_H_
