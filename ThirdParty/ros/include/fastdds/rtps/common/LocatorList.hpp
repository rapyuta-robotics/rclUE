...
public:
    /// Constructor
    Locators(const LocatorListConstIterator& it) : it_(it)
    {
    }

    /// Copy constructor
    Locators(const Locators& other) : it_(other.it_)
    {
    }
...
public:
    ...
    /// Copy constructor
    RTPS_DllAPI LocatorList(const LocatorList& list) : m_locators(list.m_locators)
    {
    }

    /// Move constructor
    RTPS_DllAPI LocatorList(LocatorList&& list) : m_locators(std::move(list.m_locators))
    {
    }
    ...
    RTPS_DllAPI bool operator==(const LocatorList& locator_list) const
    {
        if (locator_list.m_locators.size() == m_locators.size())
        {
            ...
            for (auto it = locator_list.m_locators.begin(); returnedValue && it != locator_list.m_locators.end(); ++it)
            {
                returnedValue = false;
     ...
     */
    RTPS_DllAPI void reserve(size_t num)
    {
     ...
     */
    RTPS_DllAPI void resize(size_t num)
    {
    ...
    FASTDDS_DEPRECATED_UNTIL(3, "eprosima::fastrtps::rtps::LocatorList::contains(const Locator&)", "Unused method.")
 ...
 */
inline std::ostream& operator<<(std::ostream& output, const LocatorList& locList)
{
    output << "[";
    if (!locList.empty())
    {
 ...
 */
inline std::istream& operator>>(std::istream& input, LocatorList& locList)
{
