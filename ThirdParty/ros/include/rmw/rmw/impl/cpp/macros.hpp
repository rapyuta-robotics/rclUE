...
#define RMW_TRY_PLACEMENT_NEW(Destination, BufferForNew, FailureAction, Type, ...)                      \
    ...
    {                                                                                                   \
        RMW_SET_ERROR_MSG((std::string("caught C++ exception ") + rmw::impl::cpp::demangle(exception) + \
...
#define RMW_TRY_DESTRUCTOR_FROM_WITHIN_FAILURE(Statement, Type)                                                                   \
    ...
    {                                                                                                                             \
        std::stringstream ss;                                                                                                     \
        ss << "caught C++ exception in destructor of " #Type " while handling a failure: " << rmw::impl::cpp::demangle(exception) \
           << ": " << exception.what() << ", at: " << __FILE__ << ":" << __LINE__ << '\n';                                        \
        (std::cerr << ss.str()).flush();                                                                                          \
    }                                                                                                                             \
    catch (...)                                                                                                                   \
    {                                                                                                                             \
        std::stringstream ss;                                                                                                     \
        ss << "caught unknown C++ exception in destructor of " #Type << " while handling a failure at: " << __FILE__ << ":"       \
           << __LINE__ << '\n';                                                                                                   \
        (std::cerr << ss.str()).flush();                                                                                          \
    }
    ...
    {                                                                                                                      \
        ...
        {                                                                                                                  \
            ...
            int ret = rcutils_snprintf(__msg,                                                                              \
                                       ...
                                       ElementTypeID,                                                                      \
                                       reinterpret_cast<const void*>(ElementTypeID),                                       \
                                       ExpectedTypeID,                                                                     \
                                       reinterpret_cast<const void*>(ExpectedTypeID));                                     \
            if (ret < 0)                                                                                                   \
            {                                                                                                              \
                static const char error_msg[] = "RMW_CHECK_TYPE_IDENTIFIERS_MATCH(): rcutils_snprintf() failed";           \
...
#define RMW_CHECK_TYPE_IDENTIFIERS_MATCH(ElementName, ElementTypeID, ExpectedTypeID, OnFailure)                                    \
    {                                                                                                                              \
        ...
        {                                                                                                                          \
            ...
            {                                                                                                                      \
                RMW_SET_ERROR_MSG("RMW_CHECK_TYPE_IDENTIFIERS_MATCH(): rcutils_snprintf(NULL, 0, ...) failed");                    \
                OnFailure;                                                                                                         \
            }                                                                                                                      \
            else                                                                                                                   \
            {                                                                                                                      \
                char* __msg = reinterpret_cast<char*>(rmw_allocate(__bytes_that_would_have_been_written + 1));                     \
                if (NULL == __msg)                                                                                                 \
                {                                                                                                                  \
