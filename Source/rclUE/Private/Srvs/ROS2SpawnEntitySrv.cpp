...
void UROS2SpawnEntitySrv::SetInputs(FString xml, FString RobotNamespace, FEntityStateData data)
{
    ...
    memcpy(spawn_entity_req.xml.data, TCHAR_TO_ANSI(*xml), (xml.Len()+1)*sizeof(char));
    ...
    memcpy(spawn_entity_req.robot_namespace.data, TCHAR_TO_ANSI(*RobotNamespace), (RobotNamespace.Len()+1)*sizeof(char));
...
void UROS2SpawnEntitySrv::SetOutput(bool Success, FString status_message)
{
    ...
    memcpy(spawn_entity_res.status_message.data, TCHAR_TO_ANSI(*status_message), (status_message.Len()+1)*sizeof(char));
}
