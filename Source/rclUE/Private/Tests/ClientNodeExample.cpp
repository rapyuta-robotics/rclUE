...
void AClientNodeExample::client_callback(const void* msg)
{
	...
	UE_LOG(LogTemp,
		   Log,
		   TEXT("Received service response %ld * %ld = %ld."),
		   AClientNodeExample::req.a,
		   AClientNodeExample::req.b,
		   msgin->sum);
}
...
void AClientNodeExample::BeginPlay()
{
	...
	rcl_client_t client;
	RCSOFTCHECK(
		rclc_client_init_default(&client, &node, ROSIDL_GET_SRV_TYPE_SUPPORT(ue4_interfaces, srv, AddInts), "/ue4srvexample"));
