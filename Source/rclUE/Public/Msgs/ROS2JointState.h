...
public:
  ...
  void SetROS2(sensor_msgs__msg__JointState &out_ros_data) const {
    ...
    if (!rosidl_runtime_c__String__Sequence__init(&out_ros_data.name,
                                                  Name.Num())) {
      UE_LOG(LogTemp, Error,
             TEXT("failed to create array for field out_ros_data.name  "));
    }
    ...
    if (!rosidl_runtime_c__float64__Sequence__init(&out_ros_data.position,
                                                   Position.Num())) {
      UE_LOG(LogTemp, Error,
             TEXT("failed to create array for field out_ros_data.position  "));
    }
    ...
    if (!rosidl_runtime_c__float64__Sequence__init(&out_ros_data.velocity,
                                                   Velocity.Num())) {
      UE_LOG(LogTemp, Error,
             TEXT("failed to create array for field out_ros_data.velocity  "));
    }
    UROS2Utils::ArrayUEToROSSequence<double, double>(
