...
public:
  ...
  SetFromROS2(const example_interfaces__msg__Float32MultiArray &in_ros_data) {
    ...
    UROS2Utils::SequenceROSToUEArray<float, float>(in_ros_data.data.data, Data,
  ...
  void SetROS2(example_interfaces__msg__Float32MultiArray &out_ros_data) const {
    ...
    if (!rosidl_runtime_c__float32__Sequence__init(&out_ros_data.data,
                                                   Data.Num())) {
      UE_LOG(LogTemp, Error,
             TEXT("failed to create array for field out_ros_data.data  "));
    }
    UROS2Utils::ArrayUEToROSSequence<float, float>(Data, out_ros_data.data.data,
                                                   Data.Num());
  }
};
