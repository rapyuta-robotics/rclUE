...
#include "ROS2InteractiveMarkerUpdateMsg.generated.h"

USTRUCT(Blueprintable)
struct RCLUE_API FROSInteractiveMarkerUpdate
{
	GENERATED_BODY()
...
public:
	...
	void SetFromROS2(const visualization_msgs__msg__InteractiveMarkerUpdate& in_ros_data)
	{
		...
		{
			PosesName.Emplace("");
			PosesName[i].AppendChars(in_ros_data.poses.data[i].name.data,in_ros_data.poses.data[i].name.size);
		}

		for (auto i = 0; i < in_ros_data.erases.size; ++i)
		{
			Erases.Emplace("");
			Erases[i].AppendChars(in_ros_data.erases.data[i].data,in_ros_data.erases.data[i].size);
		}
	...
	if (out_ros_data.poses.data != nullptr)
		...
		out_ros_data.poses.data = (decltype(out_ros_data.poses.data))malloc(PosesHeaderStampSec.Num() * (sizeof(PosesHeaderStampSec) + sizeof(PosesHeaderStampNanosec) + sizeof(PosesHeaderFrameId) + sizeof(PosesPosePosition) + sizeof(PosesPoseOrientation) + sizeof(PosesName)));
		...
		memcpy(out_ros_data.poses.data[i].header.frame_id.data, TCHAR_TO_UTF8(*PosesHeaderFrameId[i]), (strLength+1)*sizeof(char));
			...
			{
				FTCHARToUTF8 strUtf8( *PosesName[i] );
			int32 strLength = strUtf8.Length();
				out_ros_data.poses.data[i].name.data = (decltype(out_ros_data.poses.data[i].name.data))malloc((strLength+1)*sizeof(decltype(*out_ros_data.poses.data[i].name.data)));
		memcpy(out_ros_data.poses.data[i].name.data, TCHAR_TO_UTF8(*PosesName[i]), (strLength+1)*sizeof(char));
				out_ros_data.poses.data[i].name.size = strLength;
				out_ros_data.poses.data[i].name.capacity = strLength + 1;
			}
	...
	if (out_ros_data.erases.data != nullptr)
		...
		out_ros_data.erases.data = (decltype(out_ros_data.erases.data))malloc((Erases.Num())*sizeof(decltype(*out_ros_data.erases.data)));
		...
		{
			{
				...
				out_ros_data.erases.data[i].data = (char*)malloc((strLength+1)*sizeof(char));
				memcpy(out_ros_data.erases.data[i].data, TCHAR_TO_UTF8(*Erases[i]), (strLength+1)*sizeof(char));
...
UCLASS()
class RCLUE_API UROS2InteractiveMarkerUpdateMsg : public UROS2GenericMsg
{
	GENERATED_BODY()
...
public:
	...
	void SetMsg(const FROSInteractiveMarkerUpdate& Input);
	
  	UFUNCTION(BlueprintCallable)
	void GetMsg(FROSInteractiveMarkerUpdate& Output) const;
