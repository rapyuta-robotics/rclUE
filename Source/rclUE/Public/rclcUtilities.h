...
public:
                                           ...
                                           const int size)
    {
        ...
        // }
        // OutStrSequence = (decltype(OutStrSequence))malloc(sequenceTotalCapacity);
        // OutStrSequence->size = size;
        // OutStrSequence->capacity = size;

        for (auto i = 0; i < size; ++i)
        {
            // FTCHARToUTF8 strUtf8(*InStrList[i]);
            // const int32 strCapacity = strUtf8.Length() + 1;
            // memcpy(OutStrSequence[i].data, TCHAR_TO_UTF8(*InStrList[i]), strCapacity * sizeof(char));
            // OutStrSequence[i].size = strUtf8.Length();
            // OutStrSequence[i].capacity = strCapacity;
